...
score: 7.0
evidence: 符号因果世界模型与谓词发明
tldr: 本研究针对现有世界模型在样本效率、透明度和可扩展性方面的不足，提出了一种在线构建符号因果世界模型的框架。该框架通过元解释学习（MIL）和动态谓词发明技术，使智能体能在决策循环中持续学习并修复模型，从而构建出分层且解耦的高质量抽象概念。实验证明，该方法在处理复杂关系动力学时具有极高的样本效率，远超PPO等神经网络基线，并有效解决了命题方法的组合爆炸问题。
motivation: 传统世界模型在复杂环境中面临样本效率低、缺乏透明度以及在处理大规模关系动力学时难以扩展的挑战。
method: 提出一种集成持续学习与修复的符号框架，利用元解释学习和谓词发明技术从观测中自动构建分层的因果抽象。
result: 该方法在复杂关系领域表现出优异的可扩展性，且样本效率比传统的PPO强化学习算法高出数个数量级。
conclusion: 通过动态谓词发明构建的符号因果模型，为实现高效、可解释且具备良好泛化能力的智能体提供了一条有效路径。
---
