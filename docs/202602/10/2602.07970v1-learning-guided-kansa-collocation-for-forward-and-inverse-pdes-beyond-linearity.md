...
score: 6.0
evidence: 偏微分方程中的方程发现与逆问题
tldr: 本研究针对传统数值方法在处理偏微分方程（PDE）时面临的维度灾难和高计算成本问题，提出了一种学习引导的Kansa配点法。通过扩展CNF框架，研究实现了对多因变量和非线性PDE的正向求解、反向问题及方程发现。主要贡献包括开发了自调优技术、在基准问题上进行了验证，并对神经PDE求解器在科学模拟中的应用进行了全面综述。
motivation: 旨在解决传统数值PDE求解器在处理复杂物理现象时存在的计算效率低、维度灾难及对特定领域离散化依赖的问题。
method: 扩展了CNF框架以支持多因变量和非线性设置，并结合了学习引导的Kansa配点法与自调优技术。
result: 成功实现了针对正向和反向问题的求解器，并在多个基准测试中验证了其在方程发现和科学模拟中的有效性。
conclusion: 该方法为超越线性限制的复杂PDE求解提供了一种灵活且高效的神经科学计算新途径。
---
