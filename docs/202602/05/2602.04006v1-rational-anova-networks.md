...
score: 6.0
evidence: 可解释的函数分解与有理逼近
tldr: 针对深度神经网络中固定非线性激活函数限制可解释性以及样条函数（如KAN）计算效率低且不稳定的问题，本文提出了Rational-ANOVA Network (RAN)。该架构结合了函数ANOVA分解与Padé式有理函数逼近，通过可学习的有理单元参数化主效应和稀疏成对交互。RAN通过强制正分母确保数值稳定性，并利用ANOVA结构提供低阶交互偏置。实验表明，RAN在保持参数量一致的情况下，在函数拟合和视觉分类任务中表现优于MLP和KAN，具有更强的外推能力、稳定性和计算效率。
motivation: 现有的固定激活函数限制了模型控制力，而基于样条的可学习激活模型存在计算效率低和边界不稳定的缺陷。
method: 提出一种基于函数ANOVA分解的架构，利用带有正分母约束的可学习有理函数来建模主效应和稀疏交互项。
result: 在受控基准测试和CIFAR-10等视觉任务中，RAN在相同参数和计算预算下达到或超过了MLP及其他可学习激活基准。
conclusion: RAN是一种兼具高解释性、数值稳定性和优异外推性能的基础神经网络架构。
---

## 摘要
深度神经网络通常将非线性视为固定的原语（例如 ReLU），这限制了可解释性以及对所诱导函数类的控制粒度。虽然最近的加性模型（如 KAN）尝试使用样条函数来解决这一问题，但它们往往面临计算效率低下和边界不稳定的问题。我们提出了有理 ANOVA 网络（RAN），这是一种基于函数式 ANOVA 分解和 Padé 式有理逼近的基础架构。RAN 将 f(x) 建模为主效应和稀疏成对交互的组合，其中每个组件都由一个稳定的、可学习的有理单元进行参数化。关键在于，我们强制分母严格为正，从而避免了极点和数值不稳定，同时比多项式基函数更有效地捕捉剧烈转变和近奇异行为。这种 ANOVA 结构为数据效率和可解释性提供了显式的低阶交互偏差，而有理参数化则显著提高了外推能力。在参数和计算预算匹配的情况下，通过受控函数基准测试和视觉分类任务（如 CIFAR-10），RAN 的表现达到或超过了参数匹配的 MLP 和可学习激活基准模型，并具有更好的稳定性和吞吐量。代码可在 https://github.com/jushengzhang/Rational-ANOVA-Networks.git 获取。
