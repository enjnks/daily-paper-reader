...
score: 6.0
evidence: 使用遗传规划演化树状结构的多元激活函数
tldr: 针对神经网络在处理缺失数据时激活函数无法感知缺失信息的问题，本文提出三通道进化激活函数（3C-EA）。该方法利用遗传编程演化出能同时处理特征值、缺失指示符和归补置信度的多元激活函数，并配合ChannelProp算法将这些信号在网络深层传播。实验表明，该方法在多种缺失场景下均能显著提升分类性能，增强了模型对不完整数据的建模能力。
motivation: 传统激活函数仅关注特征值而忽略了缺失标识和置信度信息，限制了模型在处理缺失数据时的泛化表现。
method: 利用遗传编程演化出融合三维输入的激活函数，并引入ChannelProp算法根据权重幅值在层间传播缺失与置信度信号。
result: 在多种缺失类型（MCAR/MAR/MNAR）和不同缺失率的实验中，3C-EA配合ChannelProp显著优于传统方法。
conclusion: 将缺失感知与置信度机制集成到激活函数中，是提升神经网络处理不完整数据鲁棒性的有效途径。
---
